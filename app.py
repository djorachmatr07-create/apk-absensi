...
st.set_page_config(page_title="NEXA V17 CLEAN", layout="wide", page_icon="🛰️")
st.markdown("<h2>🛰️ NEXA V17 CLEAN</h2><p style='color:#9CA3AF;font-size:12px'>1 TOMBOL GENERATE ALL | ABSEN BALIK | AUTO MAJU</p>", unsafe_allow_html=True)
...
def load_data():
    ...
    if len(vals)>1:
        absen=pd.DataFrame([r[:14] for r in vals[1:]], columns=HEADER)
...
def hitung_lembur_bulat(jam_float, is_sabtu=False, is_minggu=False, is_merah=False, status="H"):
    ...
    if is_sabtu:
        ...
        sisa=jam_float-5; l15=1 if sisa>=1 else sisa; l20=sisa-1 if sisa>1 else 0; return "5.00",f"{sisa*1.5+l20*0.5:.2f}",f"{l15:.2f}",f"{l20:.2f}"
...
def get_uang_shift(id_kar, shift, jl=0):
    try:
        v=str(db_df[db_df['ID KARYAWAN']==id_kar][COL_UANG_SHIFT].values[0]).replace('Rp','').replace('.','').replace(',','').strip()
    except: v="0"
    if v in ["","nan"]: v="0"
    if v=="0": return "0"
    if jl>0: return v
...
def hitung_final(masuk_dt,pulang_dt,status_input):
    ...
    if status_input=="GH": return "7.00","0.00","0.00","0.00",f"H-S1" if 7<=masuk_dt.hour<=14 else f"H-S2","GANTI HARI",status_final
    ...
    ket="MINGGU" if masuk_dt.weekday()==6 else "SABTU" if masuk_dt.weekday()==5 else "MASUK" if jam_float>0 else "TIDAK MASUK"
    ...
    shift=status_final if status_final in ['A','L','SL'] else f"H-{base}" if jam_float<11.5 else f"H-LS1" if base=='S1' else "H-LS2"
...
with tab1:
    ...
    if row_today.empty:
        st.info(f"📅 Hari ini {today_str} - Silakan absen")
        status_pilih=st.selectbox("Status", ["H","GH","GHS","I","S","A"], key="st_masuk")
        c1,c2=st.columns(2)
        if ubah_manual:
            tgl_m=c1.date_input("TANGGAL MASUK", value=date.today(), key="tgl_m")
            jam_m=c2.time_input("JAM MASUK", value=datetime.now().time(), key="jam_m")
        else:
            tgl_m=date.today(); jam_m=datetime.now().time()
        ...
        if st.button("🟢 ABSEN MASUK SEKARANG", type="primary", use_container_width=True):
            # hapus auto L kalau ada hari ini
            vals=ws_absen.get_all_values()
            ...
            row=[id_in,nama,tgl_m.strftime('%Y-%m-%d'),masuk_dt.strftime('%H:%M:%S'),"","","0.00","0.00","0.00","0.00","-","MASUK",status_pilih,"0"]
        ...
        if r['JAM PULANG']=="" or r['JAM PULANG'] in ["0.00","0"]:
            ...
            c1,c2=st.columns(2)
            if ubah_manual:
                tgl_p=c1.date_input("TANGGAL PULANG", value=date.today(), key="tgl_p")
                jam_p=c2.time_input("JAM PULANG", value=datetime.now().time(), key="jam_p")
            else:
                tgl_p=date.today(); jam_p=datetime.now().time()
                try:
                    if jam_p < datetime.strptime(r['JAM MASUK'], '%H:%M:%S').time(): tgl_p=date.today()+timedelta(days=1)
                except: pass
            ...
            if st.button("Absen lagi besok"): st.rerun()
    ...
    if not st.session_state.login:
        ...
        if id_edit in db_df['ID KARYAWAN'].values:
            ...
            if not data_kar.empty:
                ...
                st.info(f"{row['TANGGAL MASUK']} {row['JAM MASUK']} -> {row['JAM PULANG']} | {row['SHIFT']} {row['KETERANGAN']}")
                c1,c2=st.columns(2)
                with c1:
                    tgl_e=st.date_input("Tgl Masuk", pd.to_datetime(row['TANGGAL MASUK']).date())
                    jm_e=st.time_input("Jam Masuk", datetime.strptime(row['JAM MASUK'], '%H:%M:%S').time() if row['JAM MASUK'] and row['JAM MASUK'] not in ["0.00",""] else datetime.now().time())
                with c2:
                    ...
                    jp_e=st.time_input("Jam Pulang", datetime.strptime(row['JAM PULANG'], '%H:%M:%S').time() if row['JAM PULANG'] and row['JAM PULANG'] not in ["","0.00"] else datetime.now().time())
...
with tab3:
    st.markdown("### ADMIN - 1 TOMBOL GENERATE ALL")
    st.warning("Tombol ini: Hapus data mundur salah (Agustus) + Generate tgl 6 Minggu maju + Auto Minggu/Merah sampai hari ini")
    if st.button("🚀 GENERATE ALL - FIX SEMUA (HAPUS MUNDUR + BUAT MAJU)", type="primary", use_container_width=True):
        vals=ws_absen.get_all_values()
        to_del=[]
        for i,r in enumerate(vals[1:], start=2):
        ...
        vals=ws_absen.get_all_values()
        existing=[r[2] for r in vals[1:] if len(r)>2]
        # generate hanya tgl 6 sampai kemarin yang bolong dan Minggu/Merah saja
        start_d=date(2026,9,6)
        end_d=date.today()
        cnt=0
        d=start_d
        while d < end_d:
            ...
            d+=timedelta(days=1)
        load_data.clear()
        st.success(f"Hapus {len(to_del)} mundur + Generate {cnt} maju. Tgl 6 udah ada!"); st.balloons(); st.rerun()

with tab4:
    st.markdown("### REKAP")
    mode_r=st.radio("Mode", ["21-20 Payroll","Bulan Kalender","Custom"], horizontal=True)
    c1,c2=st.columns(2)
    with c1: bulan_r=st.selectbox("Bulan", list(range(1,13)), index=datetime.now().month-1)
    with c2: tahun_r=st.number_input("Tahun", 2020, 2030, datetime.now().year)
    if mode_r=="Custom":
...
with tab5:
    ...
    id_gaji=st.selectbox("Karyawan Gaji", db_df['ID KARYAWAN'].tolist())
    if st.button("HITUNG GAJI REAL + UPDATE SHEET", type="primary", use_container_width=True):
        ...
        hadir=len(df_g[df_g['STATUS']=='H']); total_lembur=df_g['JAM LEMBUR'].astype(float).sum() if not df_g.empty else 0
        shift_malam=len(df_g[df_g['SHIFT'].astype(str).str.contains('S2|S3',na=False)]); hari_lembur=len(df_g[df_g['JAM LEMBUR']>0]) if not df_g.empty else 0
        ...
        c1,c2,c3=st.columns(3); c1.metric("Hadir", hadir); c2.metric("Lembur", f"{total_lembur:.2f}"); c3.metric("TOTAL GAJI", f"Rp {int(total_gaji):,}")
