...
st.set_page_config(page_title="NEXA v11.3", layout="wide", page_icon="🛰️")
st.markdown("""
<style>
.main-title {font-size:32px; font-weight:800; color:white;}
.sub-title {font-size:13px; color:#9CA3AF; letter-spacing:3px;}
</style>
<div class='main-title'>🛰️ NEXA ATTENDANCE PRO</div>
<div class='sub-title'>⚡ V11.3 FINAL - 🔥 JAM LEMBUR = H x1.5 + I x2.0 | 💰 LEMBUR + UANG SHIFT</div>
...
def hitung_lembur_bulat(jam_total_float, is_sabtu=False, is_minggu=False, is_merah=False, status="H"):
    try:
        jam_total_float = float(jam_total_float or 0)
    except:
        jam_total_float = 0.0
    ...
    if status == "GH":
        return "7.00","0.00","0.00","0.00"
    if status == "GHS":
        return "5.00","0.00","0.00","0.00"
    if is_sabtu:
...
def load_data():
    ...
    col_uang = None
    for c in db.columns:
        if 'SHIFT' in c.upper() and 'UANG' in c.upper():
            col_uang = c
    if not col_uang:
        col_uang = db.columns[-1]
...
def get_uang_shift(id_kar, shift, jam_lembur_float=0):
    ...
    if jam_lembur_float > 0:
        return v
    if not any(x in str(shift).upper() for x in ['S2','S3','LS1','LS2']):
        return "0"
    return v
...
def cek_keterangan(tgl_dt, jm_str="", jp_str="", jam_float=0, status="H"):
    ...
    if tgl_str in LIBUR_NASIONAL and status not in ["GH","GHS"]:
        return "LIBUR NASIONAL: " + LIBUR_NASIONAL[tgl_str]
    if jm_str and jp_str and jam_float > 0:
        if status == "GH":
            return "🔄 GANTI HARI"
        if status == "GHS":
            return "🔄 GANTI HARI SABTU"
        return "✅ MASUK"
    if status in ['A','I','S','C','TL','GH','GHS','L']:
        mapping = {"A":"❌ ALFA","I":"📝 IZIN","S":"🤒 SAKIT","C":"🏖️ CUTI","TL":"🔁 TUKAR LIBUR","GH":"🔄 GANTI HARI","GHS":"🔄 GANTI HARI SABTU","L":"💤 SHIFT LIBUR"}
        return mapping[status]
    if tgl_dt.weekday() == 6:
        return "⛱️ LIBUR MINGGU"
    if tgl_dt.weekday() == 5:
        return "📅 SABTU"
    return "💼 HARI KERJA" if jam_float > 0 else "🚫 TIDAK MASUK"

def cek_shift(masuk_dt, jam_float, ket, status):
    if 'LIBUR' in ket and status not in ["GH","GHS"]:
        return 'SL'
    if status in ['A','I','S','C','TL','GH','GHS','L']:
        return status
    if jam_float == 0:
...
with tab_absen:
    ...
    with c1:
        ...
        status_pilih=st.selectbox("📌 Status", ["H","GH","GHS","TL","I","S","C","A"], format_func=lambda x: {"H":"💼 H - HADIR","GH":"🔄 GH - GANTI HARI 7 JAM","GHS":"🔄 GHS - GANTI HARI SABTU 5 JAM","TL":"🔁 TL - TUKAR LIBUR","I":"📝 I - IZIN","S":"🤒 S - SAKIT","C":"🏖️ C - CUTI","A":"❌ A - ALFA"}[x])
    with c2:
    ...
    if st.button("💾 SIMPAN ABSEN",type="primary",use_container_width=True,disabled=not nama):
        ...
        st.success(f"✅ {status_pilih} disimpan!")
        st.balloons()
        st.rerun()
    ...
    if not st.session_state.login:
        ...
        if id_edit and id_edit in db_df['ID KARYAWAN'].values:
            ...
            if not data_kar.empty:
                pilih_tgl=st.selectbox("📅 Pilih Tanggal", data_kar.sort_values('TGL_DT',ascending=False)['TANGGAL'].tolist())
                row=data_kar[data_kar['TANGGAL']==pilih_tgl].iloc[0]
                st.info(f"⏰ {row['JAM MASUK']} - {row['JAM PULANG']} | ⏱️ LEMBUR {row['JAM LEMBUR']} = {row['LEMBUR 1.5']} x1.5 + {row['LEMBUR 2.0']} x2 | 💰 {row['UANG SHIFT']}")
                c1,c2=st.columns(2)
                with c1:
                ...
                with c2:
                    ...
                    st_e=st.selectbox("📌 Jadi Status", ["H","GH","GHS","TL","I","S","C","A","L"])
...
with tab_admin:
    ...
    st.success("✨ Rumus: G = H x 1.5 + I x 2.0 | 🔄 GH 7 jam no lembur | 🔄 GHS 5 jam no lembur | 💰 Lembur dapat uang shift")
    if st.button("🚀 ONE CLICK CALIBRATION", type="primary", use_container_width=True):
        with st.spinner("🔧 Fixing all data..."):
            vals=ws_absen.get_all_values()
            for i,r in enumerate(vals[1:], start=2):
                try:
                    ...
                    pass
            st.success("✅ DONE! 🎉 G = H x1.5 + I x2.0 | Contoh: 1+1=3.5 | Minggu 7=14.0")
            load_data.clear()
            st.balloons()
            st.rerun()
