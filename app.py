...
st.markdown("<h2>🛰️ NEXA V16 FINAL</h2><p style='color:#9CA3AF;font-size:12px'>1 HARI 1 ABSEN | 22:00=S3 | GH/GHS=H | 21-20 PAYROLL | G=H*1.5+I*2.0</p>", unsafe_allow_html=True)
...
def hitung_lembur_bulat(jam_float, is_sabtu=False, is_minggu=False, is_merah=False, status="H"):
    ...
    if status in ["GH","GHS"]:
        return ("7.00","0.00","0.00","0.00") if status=="GH" else ("5.00","0.00","0.00","0.00")
    if is_sabtu:
    ...
    if is_minggu or is_merah:
        return "0.00",f"{jam_float*2.0:.2f}","0.00",f"{jam_float:.2f}"
...
def cek_shift(masuk_dt,jam_float,ket,status):
    ...
    if jam_float >= 11.5:
        return f"H-LS1" if base=='S1' else f"H-LS2"
    return f"H-{base}"
...
def hitung(masuk_dt,pulang_dt,status_input):
    # status_input GH/GHS tetap hitung 7/5 jam tapi output status jadi H
    ...
    if status_input=="GH":
        jk,jl,l15,l20="7.00","0.00","0.00","0.00"
        ket="GANTI HARI"
        hm=masuk_dt.hour
        base='S1' if 7<=hm<=14 else 'S2' if 15<=hm<=21 else 'S3'
        shift=f"H-{base}"
    elif status_input=="GHS":
        jk,jl,l15,l20="5.00","0.00","0.00","0.00"
        ket="GANTI HARI SABTU"
        shift="H-S1"
    else:
...
with tab1:
    ...
    if id_in and nama:
        ...
        if not cek.empty:
            ...
            st.warning("1 hari 1x, edit via EDIT")
        ...
        if st.button("CALIBRATION SHIFT",use_container_width=True):
            ...
            for i,r in enumerate(vals[1:], start=2):
                try:
                    ...
                    jam_float=total-1.0 if total>6.0 else total if is_sabtu or stat_input in ["GH","GHS"] else (total-1.0 if total>=6.0 else total)
        ...
        if st.button("FIX GH GHS JADI H",use_container_width=True):
            ...
            for i,r in enumerate(vals[1:], start=2):
                if len(r)>=12 and r[11] in ["GH","GHS"]:
                    shift_baru="H-S1"
                    status_baru="H"
                    ket_baru="GANTI HARI SABTU" if r[11]=="GHS" else "GANTI HARI"
                    jk_baru="5.00" if r[11]=="GHS" else "7.00"
                    ws_absen.update(f'F{i}', [[jk_baru]])
                    ws_absen.update(f'J{i}', [[shift_baru]])
                    ws_absen.update(f'K{i}', [[ket_baru]])
                    ws_absen.update(f'L{i}', [[status_baru]])
...
with tab4:
    ...
    if not absen_df.empty:
        ...
        st.write(f"Total {len(df_f)} data")
...
with tab5:
    ...
    if st.button("HITUNG REAL", type="primary", use_container_width=True):
        if not absen_df.empty:
            df_g=absen_df[(absen_df['ID KARYAWAN']==id_gaji)&(absen_df['TGL_DT']>=pd.to_datetime(awal_g))&(absen_df['TGL_DT']<=pd.to_datetime(akhir_g))]
            # Karena GH/GHS sekarang status H, kehadiran otomatis kehitung
            hadir=len(df_g[df_g['STATUS']=='H'])
            st.success(f"{len(df_g)} hari | Hadir {hadir} (termasuk GH/GHS yang sudah jadi H)")
            ...
            c1,c2=st.columns(2)
            with c1: st.metric("Jam Kerja", f"{total_kerja:.2f}")
            with c2: st.metric("Jam Lembur G", f"{total_lembur:.2f}")
