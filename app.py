...
st.title("📍 APK ABSENSI V8.9 - NO TAMBAH KOLOM")
...
def load_data():
    ...
    for col in ['STATUS', 'JAM LEMBUR', 'JAM KERJA', 'SHIFT', 'KETERANGAN']: # HAPUS 2 KOLOM LEMBUR
    ...
    if not absen.empty:
        ...
        # TAMBAH KOLOM LEMBUR DI DATAFRAME DOANG, GA KE SHEET
...
def hitung(masuk_dt, pulang_dt, status):
    ...
    return f"{jam_kerja_float:.2f}", f"{jam_lembur_total:.2f}", shift, keterangan, jam_masuk_str, jam_pulang_str, lembur_1_5, lembur_2_0
...
def upsert_absen(id_kar, masuk_dt, pulang_dt, nama, status="H", sudah_pulang=False):
    ...
    if status in STATUS_NON_JAM:
        ...
        jam_kerja, jam_lembur, shift, ket, jam_masuk_str, jam_pulang_str, _, _ = hitung(masuk_dt, pulang_dt, status)
    row_data = [id_kar, nama, jam_masuk_str, jam_pulang_str, jam_kerja, jam_lembur, shift, ket, status] # TETEP 9 KOLOM
    ...
    if not existing.empty:
        ...
        ws_absen.update(f'A{row_num}:I{row_num}', [row_data]) # UPDATE SAMPAI I
    else:
