...
if st.button("🔍 HITUNG GAJI", type="primary", use_container_width=True):
    ...
    for id_kar in db_df['ID KARYAWAN'].unique():
        ...
        rekap.append([id_kar, nama, int(jml_hari), uang_hari, total_shift, total_l15, total_l20, total_lembur, total_shift+total_lembur])

    df = pd.DataFrame(rekap, columns=['ID','NAMA','HARI S2/S3/LS','UANG/HARI','TOTAL SHIFT','L1.5','L2.0','TOTAL LEMBUR','TOTAL'])
    st.dataframe(df, use_container_width=True)
    st.success(f"Total Shift: Rp {df['TOTAL SHIFT'].sum():,.0f} | Total Lembur: Rp {df['TOTAL LEMBUR'].sum():,.0f}")
